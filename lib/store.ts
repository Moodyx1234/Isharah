...
export const useSessionStore = create<SessionState & SessionActions>((set) => ({
  ...
  addStudent: (student) =>
    set((s) => ({ connectedStudents: [...s.connectedStudents, student] })),
  removeStudent: (id) =>
    set((s) => ({ connectedStudents: s.connectedStudents.filter((st) => st.id !== id) })),
  updateStudentHand: (id, raised) =>
    set((s) => ({
      connectedStudents: s.connectedStudents.map((st) =>
        st.id === id ? { ...st, handRaised: raised } : st
      ),
    })),
  addTranscript: (entry) =>
